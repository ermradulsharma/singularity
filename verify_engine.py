...
def test_sandbox():
    ...
    sandbox = SecureSandbox(use_docker=False) # Testing local AST filter only
    ...
    # Test 2: Malicious Code (Dunder access)
...
def test_model_degradation():
    ...
    device = "cpu"
    config = ModelArgs()
    # Tiny model for fast verification (vocab_size must be >= 50257 for the benchmark tensor)
